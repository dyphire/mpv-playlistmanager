...
This script allows you to see and interact with your playlist in an intuitive way. The key features are removing, reordering and playing files. Additional features include resolving url titles, stripping filenames according to patterns and creating/saving/shuffling/sorting/refreshing playlists.
...
This script is able to resolve titles for urls with youtube-dl. See the `resolve_url_titles`, `prefer_titles` and `youtube_dl_executable` settings for details.
...
- __refreshplaylist__
  - Refresh the current playlist. Does not stop playing the current file.
- __loadfiles__
